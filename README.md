...
- ✅ **Panneau Jarvis (P4) — voix** : reconnaissance et synthèse vocales via la Web Speech API du navigateur (STT + TTS, fr-FR), bouton micro, **activation par mot-clé « Hey Jarvis »** (écoute continue, en pause pendant que Jarvis parle), lecture des réponses, mute, orbe réactif. Repli texte si l'API n'est pas supportée. Migration possible vers Whisper local côté serveur plus tard.
  - Diagnostic Everping : `GET /api/everping` (mode + panneau avec `sourceError`), `POST /api/everping/refresh` (force une récupération).
